...
('1', '中国', 'landmark', 528.0, 116.4347, 39.9087, '北京市朝阳区CBD核心区', '朝阳区', '北京', 'normal', 108),
...
```http
GET /api/v1/map/poi/search?keyword=中国&city=北京
Authorization: Bearer {access_token}
```
...
test.describe('AI Chat Flow', () => {
  test('user can chat with AI', async ({ page }) => {
    ...
    // 发送消息
    await page.fill('[data-testid="chat-input"]', '飞到中国');
...
# 使用exists代替count
if session.query(Building.id).filter_by(name="中国").first():
    pass  # 存在

# 推荐
if session.query(
    session.query(Building).filter_by(name="中国").exists()
).scalar():
    pass
```
